...
def get_id(instances=1):
    _ids = []
    with open('painel/ids.pickle', 'rb') as load:
        currentId = pickle.load(load)
        for i in range(0, instances):
            currentId += 1
            _ids.append(currentId)
            
    with open('painel/ids.pickle', 'wb') as dump:
...
def get_icon_id():
    _ids = []
    with open('painel/icon_ids.pickle', 'rb') as load:
        currentId = pickle.load(load) + 1

    with open('painel/icon_ids.pickle', 'wb') as dump:
        pickle.dump(int(currentId), dump)
